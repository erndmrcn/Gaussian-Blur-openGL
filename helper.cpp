...
// we only use texture map in this case
void initTexture(GLuint &idJpegTexture, char *texture_map_file, int *w, int *h)
{
    int t_width, t_height;

    unsigned char *t_raw_image = NULL;
    int bytes_per_pixel = 3;   /* or 1 for GRACYSCALE images */
    int color_space = JCS_RGB; /* or JCS_GRAYSCALE for grayscale images */

    /* these are standard libjpeg structures for reading(decompression) */
    struct jpeg_decompress_struct t_cinfo;
    struct jpeg_error_mgr t_jerr;

    /* libjpeg data structure for storing one row, that is, scanline of an image */
    JSAMPROW t_row_pointer[1];
